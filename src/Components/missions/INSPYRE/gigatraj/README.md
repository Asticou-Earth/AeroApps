...
- Meteorolocal data catalog needed by ``GigaTraj``. These are the configuration files specifying the file locations and variable names for driving the trajectory code. There are versions specific to GEOS and ECMWF IFS forecats:
...
2. Update ``gigatraj.yaml`` with the addition of new fires to the ``Fires:`` dictionary, and enter the desired particle release dates/specific files under ``Trajectories:``. Also update the airports in ``inspyre.yaml`` during suitcase flights.
3. Customize ``plot_allfires`` for multiple plotting regions, valid times for density plots.
